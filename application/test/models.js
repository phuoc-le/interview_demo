...
describe('BlogPost Model', () => {
    it('should create a blog post', (done) => {
        const BlogPostMock = sinon.mock(new BlogPost({
            title: 'Test Title 1',
            content: "Test content 1",
            author: 'Test User'
        }));
        const user = BlogPostMock.object;
        ...
        user.save((err) => {
            BlogPostMock.verify();
            BlogPostMock.restore();
            expect(err).to.be.null;
            done();
        });
    });

    it('should find blog post by id', (done) => {
        const blogPostMock = sinon.mock(BlogPost);
        ...
        blogPostMock
            .expects('findOne')
            .withArgs({title: 'Test Title 1'})
            .yields(null, expectedBlogPost);

        User.findOne({title: 'Test Title 1'}, (err, result) => {
            blogPostMock.verify();
            blogPostMock.restore();
            expect(result.title).to.equal('Test Title 1');
            done();
        });
    });

    it('should remove blog post by id', (done) => {
        const blogPostMock = sinon.mock(BlogPost);
        const expectedResult = {
            _id: '66f6b8e7f249f6aead57d92c'
        };
        ...
        User.deleteOne({title: 'Test Title 1'}, (err, result) => {
            blogPostMock.verify();
            blogPostMock.restore();
            expect(err).to.be.null;
